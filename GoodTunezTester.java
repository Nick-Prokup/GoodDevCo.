...
class GoodTunezTester {
	...
	// Tester to check the functionality

	// GREY BOX TESTS BELOW
	
	// Tester method to insure that the song function creates new songs correctly
	@Test 
	void testSong() {
		Song test = new Song("Doomed", "NBA Youngboy", "Until I Return", 2021, "Hip Hop / Rap");
		
		assertEquals(test.toString(), "Doomed,NBA Youngboy,Until I Return,2021,Hip Hop / Rap");
		
		assertEquals(test.getName(), "Doomed");
		assertEquals(test.getArtist(), "NBA Youngboy");
		assertEquals(test.getAlbum(), "Until I Return");
		assertEquals(test.getYear(), 2021);
		assertEquals(test.getGenre(), "Hip Hop / Rap");
